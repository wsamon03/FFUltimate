...
BEGIN
    INSERT INTO players (espn_id, name, position_code, team_id)
        VALUES (p_espn_id, p_name, p_position_code, p_team_id)
    ON CONFLICT (espn_id) DO UPDATE SET
        name            = EXCLUDED.name,
        position_code   = EXCLUDED.position_code,
        team_id         = EXCLUDED.team_id
    WHERE players.name <> EXCLUDED.name
       OR players.position_code <> EXCLUDED.position_code
...
BEGIN
    INSERT INTO player_game_stats (
        ...
        p_rush_att, p_rush_yds, p_rush_td,
        p_rec_receptions, p_rec_targets, p_rec_yds, p_rec_td,
        p_def_solo, p_def_ast, p_def_sacks, p_def_tfl, p_def_pd, p_def_qb_hits, p_def_td, p_def_int,
        p_ret_kick_no, p_ret_kick_yds, p_ret_kick_td,
        p_ret_punt_no, p_ret_punt_yds, p_ret_punt_td,
        p_k_fg_make, p_k_fg_att, p_k_xp_make, p_k_xp_att,
    ...
    ON CONFLICT (player_id, game_id) DO UPDATE SET
        ...
        pass_sacked     = EXCLUDED.pass_sacked,
        rush_att        = EXCLUDED.rush_att,
        rush_yds        = EXCLUDED.rush_yds,
        rush_td         = EXCLUDED.rush_td,
        rec_receptions  = EXCLUDED.rec_receptions,
        rec_targets     = EXCLUDED.rec_targets,
        rec_yds         = EXCLUDED.rec_yds,
        rec_td          = EXCLUDED.rec_td,
        def_solo        = EXCLUDED.def_solo,
        def_ast         = EXCLUDED.def_ast,
        def_sacks       = EXCLUDED.def_sacks,
        def_tfl         = EXCLUDED.def_tfl,
        def_pd          = EXCLUDED.def_pd,
        def_qb_hits     = EXCLUDED.def_qb_hits,
        def_td          = EXCLUDED.def_td,
        def_int         = EXCLUDED.def_int,
        ret_kick_no     = EXCLUDED.ret_kick_no,
        ret_kick_yds    = EXCLUDED.ret_kick_yds,
        ret_kick_td     = EXCLUDED.ret_kick_td,
        ret_punt_no     = EXCLUDED.ret_punt_no,
        ret_punt_yds    = EXCLUDED.ret_punt_yds,
        ret_punt_td     = EXCLUDED.ret_punt_td,
        k_fg_make       = EXCLUDED.k_fg_make,
        k_fg_att        = EXCLUDED.k_fg_att,
        k_xp_make       = EXCLUDED.k_xp_make,
        k_xp_att        = EXCLUDED.k_xp_att,
        p_no            = EXCLUDED.p_no,
        p_yds           = EXCLUDED.p_yds,
        p_in20          = EXCLUDED.p_in20,
        p_tb            = EXCLUDED.p_tb,
        p_fc            = EXCLUDED.p_fc,
        p_blk           = EXCLUDED.p_blk,
