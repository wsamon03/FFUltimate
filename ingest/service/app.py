...
async def delete_game(event_id: str = Query(...)):
    ...
    global POOL
    try:
        # Validate ESPN event ID format (must contain at least one dash and not be empty)
        if not event_id or '-' not in event_id:
            return {"status": "error", "message": "Invalid ESPN event ID format"}
        
        async with POOL.acquire() as conn:
            game = await conn.fetchrow(
                "SELECT id FROM games WHERE espn_id = $1", event_id
            )
            
            if not game:
                return {"status": "warning", "message": "Game not found in database.", "details": {"event_id": event_id, "games_matching": 0}}

            game_id = game['id']

            game_id = game['id']
            
            # Delete player and team stats first (foreign key safety)
            ...
            await conn.execute("DELETE FROM games WHERE espn_id = $1", event_id)
            
        return {"status": "success", "message": f"Game {event_id} deleted"}
    except Exception as e:
